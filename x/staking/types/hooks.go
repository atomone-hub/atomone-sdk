...
func (h MultiStakingHooks) AfterConsensusPubKeyUpdate(ctx context.Context, oldPk, newPk cryptotypes.PubKey, fee sdk.Coin) error {
	for i := range h {
		if err := h[i].AfterConsensusPubKeyUpdate(ctx, oldPk, newPk, fee); err != nil {
			return err
		}
	}
	return nil
