...
These are absent from the v0.50.x base but originate in newer or related upstream code:
...
- **App version connected to consensus params (`baseapp`).** `SetProtocolVersion` is
  ...
  `ParamStore`. Backport of upstream cosmos-sdk #16244 (with parts of #23622 and
  #21508).
- **Consensus key rotation (`x/staking`, ADR-016).** Validators can rotate their
  consensus key via `MsgRotateConsPubKey`. Backport of the upstream ADR-016 feature.
...
- **Grant-pruning limits (`x/authz`).** At most 200 expired grants pruned per block,
  plus a `PruneExpiredGrants` message. Backport of upstream cosmos-sdk #18737.
